# props.children and component refs
...
You can check the *package.json* file to verify that version 19 of the *react* and *react-dom* libraries are in use:
...
Also run the command *npm install*, which installs the dependencies according to the *package.json* file. This is necessary if, for example, you cloned the example repository at an earlier stage of the course, when an older version of React was still in use.
...
> **Image:** A browser window showing a web application with a pink-bordered "log in" button visible by default. The rest of the page shows a list of notes. No login form fields are visible — only the single button that will reveal the form when clicked.

The login form appears when the user presses the *login* button:

> **Image:** A browser window showing the login form expanded with fields for "username" and "password", a "login" submit button, and a "cancel" button. The username field contains the text "mluukkai" and the password field is filled with masked dots. A cursor is hovering over the "cancel" button.
...
> **Image:** A browser window showing a web application with three separate Togglable components. Each has a numbered button ("1", "2", "3") and when expanded, reveals different content ("first", "second", "third") with their own cancel buttons. All three have pink borders. Each component maintains its own independent visibility state.
...
> **Image:** A browser window showing a blog list application. A pink-bordered "new note" button is visible but no form is displayed. The page shows existing blog posts with titles and authors. The form for creating new blogs is hidden by default.

It expands when button *create new blog* is clicked

> **Image:** A browser window showing the blog creation form expanded. The form contains fields for title, author, and URL with a "create" submit button and a "cancel" button. Existing blog posts are visible below the form. The form has a pink border.
...
The component must work like the *NoteForm* component from the [material](/en/part5/props_children_and_component_refs#state-of-the-forms) of this part.
...
> **Image:** A browser window showing a list of blog posts. Each blog has a "view" button. One blog is expanded showing full details including the URL, number of likes with a "like" button, and the author's name. Other blogs show only their title and author with "view" buttons. Each blog has a pink border with padding.
...
> **Image:** A browser window showing an expanded blog post. The blog displays title, URL, likes with a like button, but the author/user name field underneath is blank or missing. A pink border surrounds the blog card. This illustrates a bug where the user name disappears after liking a blog.
...
> **Image:** A browser window showing a blog post with expanded details. A confirmation dialog is visible asking the user to confirm blog removal (likely using window.confirm). The blog shows title, URL, likes, author name, and a "remove" button. Pink border surrounds the blog card.
