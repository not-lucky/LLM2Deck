...
export const DEFAULT_GENERATION_TOPIC = `You are a world-class educator and domain-specific technical expert.
...
export const DEFAULT_GENERATION = DEFAULT_GENERATION_TOPIC;
...
export function resolvePrompts(promptsConfig, subject = '', cardType = 'standard', mode = null) {
  ...
  // For topic mode, we look for generation_topic override first, then standard
  // generation override, then fallback to DEFAULT_GENERATION_TOPIC.
  let genBase;
  if (resolvedMode === 'document') {
    genBase = defaults.generation_document || defaults.generation || DEFAULT_GENERATION_DOCUMENT;
  } else {
    genBase = defaults.generation_topic || defaults.generation || DEFAULT_GENERATION_TOPIC;
  }
