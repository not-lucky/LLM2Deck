...
class CerebrasProvider(LLMProvider):
    ...
    async def _make_request(self, chat_messages: List[Dict[str, Any]], json_schema: Dict[str, Any], max_retries: int = 7) -> Optional[str]:
        for attempt_number in range(max_retries):
            try:
                api_client = self._get_client()
                optional_parameters = {}
                if self.model_name == "gpt-oss-120b":
                    optional_parameters["reasoning_effort"] = self.reasoning_effort_level
