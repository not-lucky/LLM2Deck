...
)  # Go up from config/ to src/ to project root
ARCHIVAL_DIR = BASE_DIR / "anki_cards_archival"
DATABASE_PATH = BASE_DIR / "llm2deck.db"
CEREBRAS_KEYS_FILE_PATH = Path(os.getenv("CEREBRAS_KEYS_FILE_PATH", "api_keys.json"))
OPENROUTER_KEYS_FILE = Path(
    os.getenv("OPENROUTER_KEYS_FILE_PATH", "openrouter_apikeys.json")
)
GEMINI_CREDENTIALS_FILE = Path(
    os.getenv("GEMINI_CREDENTIALS_FILE_PATH", "python3ds.json")
)
NVIDIA_KEYS_FILE = Path(os.getenv("NVIDIA_KEYS_FILE_PATH", "nvidia_keys.json"))
CANOPYWAVE_KEYS_FILE = Path(
    os.getenv("CANOPYWAVE_KEYS_FILE_PATH", "canopywave_keys.json")
)
BASETEN_KEYS_FILE = Path(os.getenv("BASETEN_KEYS_FILE_PATH", "baseten_keys.json"))
GOOGLE_GENAI_KEYS_FILE = Path(
    os.getenv("GOOGLE_GENAI_KEYS_FILE_PATH", "google_genai_keys.json")
)
